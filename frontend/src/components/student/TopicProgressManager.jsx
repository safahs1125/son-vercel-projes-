...
const TYT_SUBJECTS = {
  'Turkce': ['Anlatim', 'Sozcuk', 'Cumle', 'Paragraf', 'Edebiyat'],
  'Matematik': ['Sayilar', 'Denklemler', 'Fonksiyonlar', 'Geometri', 'Olasilik'],
  'Fen': ['Fizik', 'Kimya', 'Biyoloji'],
  'Sosyal': ['Tarih', 'Cografya', 'Felsefe', 'Din Kulturu'],
};

const AYT_SUBJECTS = {
  'Matematik': ['Integral', 'Turev', 'Limit', 'Diziler', 'Trigonometri'],
  'Fizik': ['Kuvvet', 'Elektrik', 'Manyetizma', 'Optik'],
  'Kimya': ['Asit-Baz', 'Kimyasal Denge', 'Organik Kimya'],
  'Biyoloji': ['Hucre', 'Genetik', 'Ekosistem'],
};
...
export default function TopicProgressManager({ studentId }) {
  ...
  const updateTopicStatus = async (subject, topicName, newStatus) => {
    ...
    try {
      ...
      toast.error('Guncelleme hatasi: ' + error.message);
    }
  };
  ...
  if (loading) {
    return <div className="text-center text-gray-500">Yukleniyor...</div>;
  }

  return (
    <Card className="p-6 gradient-card">
      <h3 className="text-xl font-bold text-gray-800 mb-4">Konu Ilerleme Durumu</h3>
